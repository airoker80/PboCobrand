...
import com.paybyonline.KantipurPay.Adapter.Model.BalanceRequest;
...
public class BalanceRequestListViewHolder extends RecyclerView.ViewHolder  {
    ...
    public void bind(final BalanceRequest balanceRequest){
      ...
      purchaseAmt.setText( balanceRequest.getPurchasedAmount());
        ...
        if(balanceRequest.getflatPercent().toString().length()>0){
          flatPercent
                    .setText( balanceRequest.getflatPercent());}
        if( balanceRequest.getflatDiscount().toString().length()>0){
            wdDisCom
        ...
        if(balanceRequest.getDiscountAmount().toString().length()>0){
           ptDisCom
